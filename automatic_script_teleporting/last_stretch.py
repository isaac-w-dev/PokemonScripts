...
def get_steps(location_name, location_array):
    ...
    if backward_allowed:
        b_coordinates, steps, direction = find_closest_point(index, b_jp, r_direction, filter)
        shortest_coordinates, steps, direction = forward_or_backward(shortest_coordinates, [b_coordinates, steps], filter, location_name)
