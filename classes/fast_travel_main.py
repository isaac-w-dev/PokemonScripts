...
class main():
    ...
    def write_wait_combo(self):
        ...
        new_string = block(self.g.start_combo('wait_combo'),
                        call("wait", [6000]))
        return new_string
        ...
        new_string = if_block(self.g.start_function("get_destination", []),
                        if_block(start_block(f'get_val({gpc.zl})'),
                            ...
                            if_block(start_block(f'category != 0 && location < 0'),
                                reassign('location', 'num_per_category[category - 1] - 1')),
    ...
    #     new_string = self.g.create_full_block(self.g.start_combo('down_input'),
    #                                         self.g.button_input(gpc.dd))
    #     new_string += self.g.create_full_block(self.g.start_combo('up_input'),
    #                                         self.g.button_input(gpc.du))
    #     new_string += self.g.create_full_block(self.g.start_combo('left_input'),
    #                                         self.g.button_input(gpc.dl))
    #     new_string += self.g.create_full_block(self.g.start_combo('right_input'),
    #                                         self.g.button_input(gpc.dr))
    #     return new_string
        
    def write_menu_select(self):
        new_string = self.g.create_full_block(self.g.start_combo('menu_select'), self.g.button_sequence(gpc.plus, gpc.yb))
        return new_string

    def write_category_navigation(self):
        combo = self.g.call_combo

        new_string = self.g.create_full_block(self.g.start_combo('select_category'),
                                                self.g.button_input(gpc.min),
    ...
    def write_coordinate_navigation(self):
        ...
        combo = self.g.call_combo
        combo_firstline = self.g.start_combo
        start_if = self.g.start_if_block
        start_else = self.g.start_else
        input = (self.g.button_input)
        new_string = block(combo_firstline('coordinate_navigation'),
                        ...
                        block(start_if("SCRIPT_RUNNING"),
                            ...
                            block(start_if('stage == 2 && !combo_running(menu_select)'),
                                block(start_if('first_loop'),
                                    ...
                                    assign('stage', 3)),
                            block(start_if('stage == 3 && !combo_running(select_category)'),
                                combo("coordinate_navigation"),
                                assign('stage', 4)),
                            block(start_if('stage == 4 && !combo_running(coordinate_navigation)'),
                            combo('wait_combo'),
                            assign('stage', 0))),
